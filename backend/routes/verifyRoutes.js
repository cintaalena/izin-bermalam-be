...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      let userLocalHour = (utcNow.getUTCHours() + timeZoneOffset) % 24;
      let userLocalDate = new Date(utcNow);
      ...
      if (!isValidTime) {
        ...
        let localMorningStart = (6 + (timeZoneOffset - 7)) % 24;
        let localMorningEnd = (12 + (timeZoneOffset - 7)) % 24;
        let localEveningStart = (18 + (timeZoneOffset - 7)) % 24;
        let localEveningEnd = (23 + (timeZoneOffset - 7)) % 24;

        // Handle day boundary crossings for display
        let morningWindow = `${localMorningStart}:00-${localMorningEnd}:00`;
